...
v1 (init / import / search / publish / status) 実装済み。 remote 側の変更取り込み (pull)・整合性修復
(doctor)・日常操作の糖衣が未実装。
...
- [x] `gistan pull [path]` — remote-drift の取り込み。conflict は diff 提示 → remote 適用 or skip
      (local 温存 → publish で押し返す)。`--stars` は v3 送り — 260706
- [x] `gistan doctor` — 孤児 gist (file-missing) の復元 or 削除、upstream 削除済み gist の unlink
      を対話修復 — 260706
...
- [x] `gistan sync` — add / commit / pull --rebase / push (remote 未設定なら local commit のみ) —
      260706
- [x] `gistan root` — 260706

## testcases

- [x] pull: remote-drift の自動適用 / conflict の拒否 (local 温存) と承認 (remote 上書き) / upstream
      削除の警告
...
- pull / doctor は reconcile (照合エンジン) の結果への「適用」だけを書く。drift
  判定ロジックの再実装禁止 (SPEC-0001)
- conflict 解決は「remote で上書き or skip」の 2 択に簡略化 (skip + publish が SPEC の「local
  優先」に相当)
