...
> **Status: Closed (2026-07-12)**。本 TASK の残タスクは全て
> [TASK-999999-backlogs](./TASK-999999-backlogs.md) へ棚卸し済み。**次の作業は backlogs から pick
> して新 TASK を作る** — このファイルはクローズ時点 (v0.7.0) のスナップショット記録。
...
- 実データ (約 753 gists) の import 済み・日常利用中。実機フィードバックは
  TASK-260708-cli-ux-improvements の followup 1〜4 として反映済み。v0.7.0 分の実機確認は backlogs
  参照
...
- ドキュメントは 2026-07-12 に棚卸し済み: ADR-0003 追加 (dirname 廃止 / description の index 化)、
  SPEC-0001 は現状と一致、過去 TASK は全てクローズ or ステータス注記済み、未解決事項は
  TASK-999999-backlogs に一元化
...
このファイルと backlogs を読めば、次の作業をどこから始めればいいか迷わない状態。

## todo

クローズ時点の決着。未完了分は全て [TASK-999999-backlogs](./TASK-999999-backlogs.md) へ移管。

0. **search レイテンシ改善** → **未着手のまま backlogs へ移管**。
   [TASK-260712-search-latency](./TASK-260712-search-latency.md) の指示書は有効だが、v0.7.0 で
   renderer に description マッチが加わったため asis 実測は要更新 (詳細は backlogs)
1. **star 機能の実機確認 3 件** → 未実施のまま backlogs へ移管
2. **配布の強化** → goreleaser 化は完了、残りは backlogs へ移管
   ...
   - mise インストール実機確認 / 上流 registry / tag-trigger 自動化 / attestations → backlogs
3. **運用で実害が出たら着手** → SPEC-0001 Open Questions ごと backlogs へ移管
...
4. **(本 TASK 作成後に追加着手した分) dirname 廃止 / アーキテクチャ刷新** → **完了 (v0.7.0)**。
   ...
   - 実機フィードバックのみ未実施 → backlogs

## testcases

- [x] 新セッションでこのファイル + backlogs + SPEC-0001 を読めば、現状把握と作業着手ができる
