...
> ADR-0003 の再設計 (gists/<gist-id>/ 構成・index v3・push / pull) は TASK-260712
> で実装中。現行バイナリ (v0.6.0) は旧仕様 (dirname / .description.txt / index v2) で動作する。
...
- `new filename.md` は local-id を採番して `gists/<local-id>/filename.md` を作る。`new dir/file.md`
  は受け付けない (gist に階層が存在しないため)。既存 gist へのファイル追加は `--id <id|local-id>`
  で行う。`-d` の値は index (`locals`、`--id` が published を指す場合は publish 時反映用)
  に保存する。`--publish` は作成後そのまま gist 化まで行う (可視性は default secret、`--public` で
  public)。終了時に実体 path と id (published なら gist URL) を必ず表示する
- `publish <id|url>` は dir 全体を gist として作成・更新する。**id / URL 指定のみ**
  ...
  を送る。`-d` は description を index に保存した上で remote へ反映する。可視性変更は delete +
  recreate = **URL が変わる** ため確認必須。新規作成のデフォルト可視性は secret。public は
  `--public` の明示指定のみ (誤公開防止。`gh gist create` のデフォルトとも一致)
...
- `pull` は remote drift を全列挙 → 対話確認 (yes で一括取り込み)。conflict / remote-deleted は skip
  して `status` へ誘導する。旧仕様の per-dir `pull [dirname]` は廃止
