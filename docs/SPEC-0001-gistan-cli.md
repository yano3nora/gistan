...
- `status` は既定 offline。`--remote` で remote drift を見る。`--fix` は旧 doctor 相当の対話修復で、doctor コマンドは存在しない
...
- **gh 未認証・ネットワーク断**: remote を触るコマンドのみ失敗し、明確なエラーを出す。local 系コマンド (`search` / `edit` / `list` / `root`) は常に動く。`root push` / `root pull` は例外で、GitHub origin と通信するため git 自身のネットワークエラーで失敗しうる
